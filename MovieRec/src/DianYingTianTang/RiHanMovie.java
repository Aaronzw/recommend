...
//		while (true) {
		...
		Vector <String> nameOfMovieIntxt = getStringFromTxt.GetStringFromTxToVector("D://课程设计//电影天堂//日韩电影//AllMovie.txt");
		...
		for (int i = 0; i < 10; i++) {
			String url = "http://www.ygdy8.net/html/gndy/rihan/list_6_" + 
					 String.valueOf(i + 1) + ".html";
			newmovie[i] = new GetMovieByUrl(url, 3);
		}
		...
		for (int i = 0; i < allMovie.size(); i++) {
			...
			thread.addElement(new Thread(new downLoadPhoto(allMovie.get(i), "D://课程设计//电影天堂//日韩电影")));
